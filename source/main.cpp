...
auto write_tokens(const fs::path& source_path, const TokenCollection& tokens)
    -> void {
#ifdef TRACE
  ...
  auto token_file =
      std::ofstream{token_path, std::ofstream::out | std::ofstream::trunc};
...
auto write_syntax(const fs::path& source_path, const SourceNode& source)
    -> void {
#ifdef TRACE
  const auto rel_path = fs::relative(source_path, src_dir_path);
  const auto syntax_path =
      syn_dir_path / (rel_path.stem().concat(syn_file_ext));
  fs::create_directories(syn_dir_path);

  auto syntax_file =
      std::ofstream{syntax_path, std::ofstream::out | std::ofstream::trunc};
  source.serialize(syntax_file, "Source");
#endif
}

auto compile(const fs::path& file_path) -> void {
  std::cout << "Source File : " << file_path << std::endl;
